...
# Use mawk-compatible syntax
mawk -v start_epoch="$START_EPOCH" -v total="$TOTAL_LINES" -v step="$PROGRESS_STEP" '
...
function format_duration(sec) {
  ...
  if (s > 0 || result == "") result = result s "s"
  gsub(/ $/, "", result)
  return result
}

function update_progress(current, total, last_p) {
  if (total <= 0) return last_p
